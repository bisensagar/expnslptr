...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        {loading ? (
          ...
          <div className="grid-2">
            {trips.map(trip => (
              <div key={trip.id} className="trip-card" onClick={() => navigate(`/admin/trips/${trip.id}`)}>
      ...
      {showTripModal && <TripModal onClose={() => setShowTripModal(false)} onSaved={fetchData} />}
      {showUserModal && <UserModal onClose={() => setShowUserModal(false)} onSaved={fetchData} />}
    </div>
  )
}
...
// ── CREATE USER MODAL ──────────────────────────────────────────────────────
// Uses signUp so it works from the browser without service-role key.
// Supabase will send a confirmation email — we set email_confirm via DB trigger.
// For instant access without email confirm: disable "Confirm email" in
...
function UserModal({ onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    const { data, error: signUpErr } = await supabase.auth.signUp({
      ...
      options: {
        data: { name: name.trim(), is_admin: false }
      }
    })
    ...
    // Step 2: If user already confirmed (no email confirm flow),
    // the profile trigger fires automatically.
    // If email confirm is ON, the profile is created on first login.
    // Either way, we upsert the profile now to ensure name is set.
    if (data?.user?.id) {
    ...
    setSaving(false)
    setSuccess(`✅ User "${name}" created! Login: ${email.trim().toLowerCase()} / ${password.trim()}`)
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div style={{
          ...
          💡 User will log in with these credentials. Share them directly — no email needed.
        </div>
